...
static esp_err_t panel_st7789_reset(esp_lcd_panel_t *panel)
{
    ...
    if (st7789->reset_gpio_num >= 0) {
        ...
        vTaskDelay(pdMS_TO_TICKS(120)); // ST7789 spec: wait at least 120ms after SWRESET before sending commands
    }
...
static const st7789_lcd_init_cmd_t vendor_specific_init_default[] = {
    ...
    // Note: SLPOUT(0x11), NORON(0x13), MADCTL(0x36), COLMOD(0x3A) are sent in panel_st7789_init before this array
    
    // display and color format setting
    {0xB6, (uint8_t []){ 0x0A, 0x82 }, 2, 0},       // JLX240 display
    {0xB0, (uint8_t []){ 0x00, 0xE0 }, 2, 0},       // ST7789_RAMCTRL, 5to6-bit conversion

    {0x3A, (uint8_t []){ 0x55 }, 1, 10},             // COLMOD, set 16 bits per pixel

    // Frame rate setting
    {0xB2, (uint8_t []){ 0x0C, 0x0C, 0x00, 0x33, 0x33 }, 5, 0},  // ST7789_PORCTRL
    {0xB7, (uint8_t []){ 0x35 }, 1, 0},             // ST7789_GCTRL, VGH/VGL

    // Power setting
    {0xBB, (uint8_t []){ 0x28 }, 1, 0},             // ST7789_VCOMS
    {0xC0, (uint8_t []){ 0x0C }, 1, 0},             // ST7789_LCMCTRL
    {0xC2, (uint8_t []){ 0x01, 0xFF }, 2, 0},       // ST7789_VDVVRHEN
    {0xC3, (uint8_t []){ 0x10 }, 1, 0},             // ST7789_VRHS
    {0xC4, (uint8_t []){ 0x20 }, 1, 0},             // ST7789_VDVSET
    {0xC6, (uint8_t []){ 0x0F }, 1, 0},             // ST7789_FRCTR2
    {0xD0, (uint8_t []){ 0xA4, 0xA1 }, 2, 0},       // ST7789_PWCTRL1

    // Gamma setting
    {0xE0, (uint8_t []){ 0xD0, 0x00, 0x02, 0x07, 0x0A, 0x28, 0x32, 0x44, 0x42, 0x06, 0x0E, 0x12, 0x14, 0x17 }, 14, 0}, // ST7789_PVGAMCTRL
    {0xE1, (uint8_t []){ 0xD0, 0x00, 0x02, 0x07, 0x0A, 0x28, 0x31, 0x54, 0x47, 0x0E, 0x1C, 0x17, 0x1B, 0x1E }, 14, 0}, // ST7789_NVGAMCTRL

    {0x2A, (uint8_t []){ 0x00, 0x00, 0x00, 0xEF }, 4, 0},             // CASET, column address set
    {0x2B, (uint8_t []){ 0x00, 0x00, 0x01, 0x3F }, 4, 0},             // RASET, row address set
};
